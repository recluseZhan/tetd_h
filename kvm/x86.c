...
int kvm_arch_vcpu_ioctl_run(struct kvm_vcpu *vcpu)
{
  /*
    ...
    */    
	...
	/*
        unsigned long *data_ptr = (unsigned long *)hva;
        unsigned long data_value = 0;
        if (virt_addr_valid((void *)hva)) {
            data_value = *data_ptr;  
            pr_info("Value at HVA (0x%lx): 0x%lx\n", hva, data_value);
        } else {
            pr_err("Failed to access HVA: 0x%lx\n", hva);
        }*/ 	
        //
